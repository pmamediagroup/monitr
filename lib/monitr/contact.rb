module Monitr
  ...
  class Contact
    ...
    def self.generate(kind)
      ...
      c = Monitr::Contacts.const_get(sym).new
      
      unless c.kind_of?(Contact)
        abort "Contact '#{c.class.name}' must subclass Monitr::Contact" 
      end
      
      c
    rescue NameError
      raise NoSuchContactError.new("No Contact found with the class name Monitr::Contacts::#{sym}")
    end
