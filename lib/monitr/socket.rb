...
module Monitr
  ...
  # The Monitr::Server oversees the DRb server which dishes out info on this Monitr daemon.
  class Socket
    attr_reader :port
    ...
    # Forward API calls to Monitr
    #
    # Returns whatever the forwarded call returns
    def method_missing(*args, &block)
      Monitr.send(*args, &block)
    end
    ...
    # Start the DRb server. Abort if there is already a running monitr instance
    # on the socket.
    #
    # Returns nothing
    def start
      begin
        ...
        begin
          ...
          abort "Socket #{self.socket} already in use by another instance of monitr"
