module Monitr
  ...
  class Logger < SimpleLogger
    ...
    def log(watch, level, text)
      ...
      self.logs[watch.name] ||= Timeline.new(Monitr::LOG_BUFFER_SIZE_DEFAULT) if watch
    ...
    def watch_log_since(watch_name, since)
      # initialize watch log if necessary
      self.logs[watch_name] ||= Timeline.new(Monitr::LOG_BUFFER_SIZE_DEFAULT)
