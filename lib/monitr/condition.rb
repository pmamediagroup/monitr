module Monitr
  ...
  class Condition < Behavior
    ...
    #   +kind+ is the underscored symbol representing the class (e.g. :foo_bar for Monitr::Conditions::FooBar)
    def self.generate(kind, watch)
      sym = kind.to_s.capitalize.gsub(/_(.)/){$1.upcase}.intern
      c = Monitr::Conditions.const_get(sym).new
      
      unless c.kind_of?(PollCondition) || c.kind_of?(EventCondition) || c.kind_of?(TriggerCondition)
        abort "Condition '#{c.class.name}' must subclass Monitr::PollCondition, Monitr::EventCondition, or Monitr::TriggerCondition" 
      end
      ...
      raise NoSuchConditionError.new("No Condition found with the class name Monitr::Conditions::#{sym}")
    end
